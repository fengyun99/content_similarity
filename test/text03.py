...
class MainWindow(QWidget):
    ...
    def compare_files(self):
        try:
            ...
            if len(highlight_list) == 0:
                QMessageBox.warning(self, "对比结果", "文件相同")
            else:
                QMessageBox.warning(self, "对比结果", "文件不同")

        except Exception:
            QMessageBox.warning(self, "错误", "请先上传两个文件")
    ...
    def clear_left_highlight(self):
        ...
        cursor.setPosition(len(self.left_upload_widget.text_edit.toPlainText()), QTextCursor.KeepAnchor)
    ...
    def clear_right_highlight(self):
        ...
        format_.setBackground(QColor("white"))
        cursor.setPosition(0, QTextCursor.MoveAnchor)
        cursor.setPosition(len(self.right_upload_widget.text_edit.toPlainText()), QTextCursor.KeepAnchor)
        cursor.mergeCharFormat(format_)
